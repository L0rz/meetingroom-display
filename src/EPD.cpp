...
void EPD_ReadBusy(void)
{
  ...
  while (1)
  {
    ...
    if (millis() - t > 20000) break;  // Timeout 20s (Full-Refresh weiß→schwarz dauert 8-12s)
    delay(10);
  }
}
...
void EPD_RESET(void)
{
  ...
  delay(20);
  EPD_RES_Set();
  delay(200);  // längeres Warten nach Reset
}
...
{
  EPD_WR_REG(0x22);
  EPD_WR_DATA8(0xFF);  // Partial Update Waveform wie EPD_Update (Full Refresh)
  EPD_WR_REG(0x20);
  EPD_ReadBusy();
}
...
{
  // Byte-für-Byte wie EPD_Clear() — bewährt, stabil
  uint16_t i, j;
  uint16_t Width  = (EPD_W % 8 == 0) ? (EPD_W / 8) : (EPD_W / 8 + 1);
  uint16_t Height = EPD_H;

  // Alten Frame (0x26) = weiß
  EPD_SetCursor(0, 0);
  EPD_WR_REG(0x26);
  for (j = 0; j < Height; j++)
    for (i = 0; i < Width; i++)
...
void EPD_Display_Part(uint16_t x, uint16_t y, uint16_t sizex, uint16_t sizey, const uint8_t *Image)
{
  ...
  EPD_SetCursor(x, y);
  EPD_WR_REG(0x24);
  // Buffer in einer SPI-Transaktion senden (kein Byte-per-Byte overhead)
  EPD_WR_DATA_BUF(Image, (size_t)Width * Height);
  EPD_Update_Part();
}
